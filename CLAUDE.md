...
| `src/context/skills/` | Phase 1/3/5 wrapper 스킬 (`harness-phase-{1,3,5}-*.md`). PR #15로 assembler inline 경로가 완성되어 **실 런타임에 반영됨**. BUG-B invariant 및 Phase 1 ambiguity 해소 정책(개발자와 live 인터랙션)이 여기에서 적용된다. |
...
| `src/phases/runner.ts` | Phase 러너 dispatcher. PR #16부터 Claude interactive 페이즈 4개 실자 `phase_end` 발행 지점(completed / artifact-commit 실패 / normal failed / catch-throw)에 `claudeTokens` 부착. redirected-by-signal 브랜치는 의도적으로 생략. **same-phase same-session 정책**: `handleInteractivePhase` 상단에서 reopen + JSONL 존재 + preset 호환 조건으로 `--resume <attemptId>` vs 신규 UUID `--session-id` 분기. runner dispatch 직전 `phase-<N>.done` 선삭제+부재 재확인(hard prerequisite, R5/D5). `state.phaseClaudeSessions['1'|'3'|'5']`에 launch preset 기록. |
