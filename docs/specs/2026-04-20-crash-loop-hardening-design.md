...
- Gate 2 재리뷰 피드백(v4 iteration):
  - **P1**: preflight이 synthesized-failure 브랜치에서 pre-UI exit을 만드므로 `enterFailedTerminalState` 앞으로 이동(preflight도 synthesized 브랜치에서 skip). R3 무조건성 유지.
  - **P2**: D3/Task 2의 `=== 'x' || 'y'` 축약 표기를 `phaseStatus === 'x' || phaseStatus === 'y'` 형태의 full-expanded comparison으로 재작성(런타임 truthy 버그 방지).
...
- **라이브 경로**(`phase-harness resume` → `src/commands/inner.ts`): 조합 감지 시 `promptModelConfig`, `invalidatePhaseSessionsOnPresetChange`, **그리고 `runRunnerAwarePreflight`를 모두 skip**한다 — 이 순서의 어떤 pre-UI 실패 경로(config prompt 블록, preflight 실패 후 `onConfigCancel` silent exit)에도 사용자가 걸리지 않도록 하기 위함. InputManager는 정상적으로 `start`된 뒤 `enterPhaseLoop` 모드로 전환되고, `runPhaseLoop`을 호출하지 않은 채 **`enterFailedTerminalState`가 즉시** 호출되어 R/J/Q 화면을 렌더한다. 사용자가 R을 선택 시 `performResume`가 수행되며, 이 시점에 runner 바이너리가 없어 실패하면 기존 runner-error 경로로 수렴(terminal UI 재진입). R3는 무조건적이다: preflight 실패·missing binary 같은 환경 문제와 무관하게 R/J/Q 화면이 먼저 렌더된다.
...
이중 방어 구조. 조건문은 반드시 **full-expanded comparison**으로 작성해야 한다(Gate 2 P2 대응 — `=== 'a' || 'b'` 축약은 `'b'`가 항상 truthy라 의도된 분기가 되지 않는다).
- **loop top**: `state.phases[phaseKey] === 'skipped'` 조기 continue 블록 옆에 아래 조건으로 즉시 `return`:
  ```ts
  const phaseStatus = state.phases[phaseKey];
  if (phaseStatus === 'failed' || phaseStatus === 'error') return;
  ```
- **각 handler 직후**: 기존 `if (state.status === 'paused') return;` 직후에 동일 형태로:
  ```ts
  const phaseStatus = state.phases[String(phase)];
  if (phaseStatus === 'failed' || phaseStatus === 'error') return;
  ```
...
7. **Conditional branch (config prompt + preflight)**:
   - `synthesizedFailure === false` → 기존 Step 5.7~5.9 전체 실행: remainingPhases 계산, `promptModelConfig`, `invalidatePhaseSessionsOnPresetChange`, 그리고 `runRunnerAwarePreflight`.
   - `synthesizedFailure === true` → **세 호출 모두 skip**. 즉 `promptModelConfig`, preset invalidation, 그리고 **`runRunnerAwarePreflight`까지 건너뛴다**. 이는 Gate 2 P1 피드백 반영: preflight 실패 시 기존 `onConfigCancel` 경로가 UI 렌더 없이 paused 상태로 조용히 exit 하기 때문에, R3의 "반드시 R/J/Q 화면이 렌더"라는 무조건성 계약이 깨진다. Synthesized 브랜치에서는 preflight을 **전혀** 호출하지 않아 pre-UI exit 가능성을 원천 제거한다.
8. Footer ticker 시작, `inputManager.enterPhaseLoop()` 전환 (기존).
9. **Conditional branch (메인 작업)**:
    - `synthesizedFailure === false` → 기존 `runPhaseLoop` + post-loop classifier 흐름 전체.
    - `synthesizedFailure === true` → `runPhaseLoop`과 preflight 모두 건너뛴 상태에서 바로 `enterFailedTerminalState(state, harnessDir, runDir, cwd, inputManager, logger)` 호출. 반환 후 기존 post-loop classifier와 동일하게 `state.status` 기반으로 `sessionEndStatus` 결정(`completed` → `enterIdle`, `paused` → sessionEndStatus='paused', 그 외 → 'interrupted'). 사용자가 R을 선택해 `performResume`가 새로이 `runPhaseLoop`을 도는 시점에 runner spawn이 실패하면 기존 runner error 처리 경로(phase_end failed + terminal UI 재진입)로 자연히 수렴한다 — preflight 부재로 인한 degradation은 이 재진입에서 가시화되므로 pre-UI silent exit 문제는 없다.
10. 기존 finally 블록(logger/footer/input stop + release lock) 동일하게 실행.
...
- 라이브 경로 `synthesizedFailure === true` 브랜치는 `promptModelConfig` + `runRunnerAwarePreflight`를 모두 건너뛰므로 `state.phasePresets`는 이전 run의 값을 그대로 재사용한다. R 선택 시 `performResume`의 `runPhaseLoop`가 해당 presets으로 동작한다. 바이너리 누락 등으로 runner가 spawn 단계에서 실패하면 기존 runner-error 경로(phase `failed` + terminal UI 재진입)로 수렴하므로, preflight 생략은 "에러 가시성"을 늦추는 trade-off이지 silent failure를 만들지 않는다.
- `runRunnerAwarePreflight`는 일반 브랜치에서만 기존대로 실행된다. 바이너리 누락 시 기존 `onConfigCancel` 경로로 폴백하는 흐름도 일반 브랜치에 한해 유지된다(R3의 무조건성은 synthesized 브랜치에서만 요구됨).
...
- **Task 2 — 루프 종료 조건에 phase error/failed 추가**
  - [ ] `src/phases/runner.ts` `runPhaseLoop` loop top에 다음 형태로 즉시 `return` 추가 (full-expanded comparison, Gate 2 P2 반영):
    ```ts
    const phaseStatus = state.phases[phaseKey];
    if (phaseStatus === 'failed' || phaseStatus === 'error') return;
    ```
    (`skipped` 분기 대칭 위치)
  - [ ] 각 phase handler 호출 직후 기존 `if (state.status === 'paused') return;` 바로 뒤에 동일 패턴으로 추가:
    ```ts
    const phaseStatus = state.phases[String(phase)];
    if (phaseStatus === 'failed' || phaseStatus === 'error') return;
    ```
...
- **Task 3 — resume paused+null 복구: 라이브 short-circuit + contracted clarified exit**
  ...
  - [ ] `innerCommand`의 Step 5.7/5.8(`promptModelConfig` + `invalidatePhaseSessionsOnPresetChange`) **및 Step 5.9(`runRunnerAwarePreflight`)**를 `!synthesizedFailure` 가드로 감싼다 (Gate 2 v4 P1 반영 — preflight 실패로 인한 pre-UI exit 차단).
  ...
  - [ ] 라이브 경로 integration 테스트 추가: `paused + pendingAction=null` state.json으로 inner 구동 → `promptModelConfig` / `runRunnerAwarePreflight`이 호출되지 않고 `enterFailedTerminalState`가 호출되는지(spy) 확인.
