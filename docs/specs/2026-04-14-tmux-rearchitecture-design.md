...
- Handoff timeout (5초 내 inner 미시작): outer가 에러 출력 + tmux 세션 kill + lock release + exit(1).

**[ADR-9] `skip`/`jump`는 lock을 획득하지 않는 control-plane 명령어다.**
- `status`/`list`: state.json 읽기 전용 — 변경 없음.
- `skip`/`jump`: lock을 획득하지 않음. 대신:
  1. `<runDir>/pending-action.json` 파일에 `{ action: 'skip' }` 또는 `{ action: 'jump', phase: N }` 기록.
  2. Lock 파일에서 inner PID 읽기 → `process.kill(innerPid, 'SIGUSR1')` 전송.
  3. Inner가 SIGUSR1 수신 시 `pending-action.json` 읽기 → state 변경 → phase loop 재진입.
- Inner PID가 없거나 dead인 경우: `pending-action.json`만 기록하고 "다음 `harness resume` 시 반영됩니다" 안내. Resume 시 inner가 시작할 때 pending action 확인.
...
export async function resumeCommand(runId, options): Promise<void> {
  ...
  const innerAlive = isInnerProcessAlive(harnessDir);  // lock의 cliPid가 alive인지 체크
  
  if (sessionExists(sessionName) && innerAlive) {
    // Case 1: 세션 + inner 둘 다 살아있음 → re-attach만
...
interface HarnessState {
  ...
  tmuxMode: 'dedicated' | 'reused';
  tmuxWindows: string[];      // harness가 생성한 tmux window ID 목록 (@0, @1, ...) — cleanup용
  tmuxControlWindow: string;  // control window의 tmux window ID
  tmuxOriginalWindow?: string; // reused mode에서 원래 활성 window ID (복구용)
}
```
...
- `tests/integration/dead-inner-recovery.test.ts` — tmux 세션 alive + inner dead → resume restarts inner
