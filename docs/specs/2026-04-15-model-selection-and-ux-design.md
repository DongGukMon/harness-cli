...
`isReopen`은 caller가 결정:
- `handleGateReject()` → `reopen_phase` pendingAction 설정 시 `isReopen = true`로 기록
- `handleVerifyFail()` → 동일
- phase 루프에서 `preparePhase()` 호출 시 `state.phases[phase] === 'completed'`이면 reopen
- **pendingAction에 의존하지 않음** — `replayPendingAction()`이 pendingAction을 clear한 후에도 정확하게 동작

대안으로 `HarnessState`에 `phaseReopenFlags: Record<string, boolean>`을 추가하여 어떤 phase가 reopen인지 영구 저장. crash/resume 경계에서도 유지됨.
...
`promptModelConfig()`도 동일: 남은 phase + pendingAction.targetPhase를 편집 가능하게 표시.
...
- `src/types.ts` — `HarnessState.phasePresets`, `HarnessState.lastWorkspacePid`, `HarnessState.lastWorkspacePidStartTime` 추가; `codexPath` nullable; `PauseReason`에 `'config-cancel'` 추가; `PendingActionType`에 `'reopen_config'` 추가
