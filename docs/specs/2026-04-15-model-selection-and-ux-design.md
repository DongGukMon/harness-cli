...
```
harness run "task"
  1. 기본 preflight (node, tmux, tty, platform, verifyScript, jq) — runner 무관 공통 항목 (Phase 6 의존성 포함). git은 현재와 동일하게 조건부 검증 (non-git repo 지원 유지)
...
**Resume 경로 분담:**
- `resume.ts`의 기존 `resumeRun()` 로직(artifact ancestry 검증, external commit 감지, typed pendingAction replay, sidecar 소비)은 **그대로 유지**한다. 이 로직은 crash-recovery 보장에 필수적이다.
- `inner.ts`에 resume branch를 추가: `resumeRun()` 호출 → model selection → runner preflight → phase loop.
- 즉 `inner.ts`가 `resumeRun()`을 **호출**하고, 그 뒤에 model selection과 preflight를 추가하는 구조.
- `resume.ts` 자체에는 model selection/preflight 로직을 넣지 않는다 — inner.ts가 단일 orchestrator.
...
**lifecycle:**
- **Set**: reopen이 발생하는 모든 경로에서 — `handleGateReject()`, `handleVerifyFail()`, `handleGateEscalation()` Continue, `handleVerifyEscalation()` Continue — target interactive phase를 `pending`으로 전환할 때:
  ```ts
...
### 테스트 변경
- `tests/conformance/phase-models.test.ts` — `PHASE_MODELS`/`PHASE_EFFORTS` → `MODEL_PRESETS`/`PHASE_DEFAULTS` 참조 변경
- `tests/state.test.ts` — `createInitialState()` 시그니처 변경, `migrateState()` 테스트 추가 (phasePresets, phaseReopenFlags, codexPath nullable)
- `tests/preflight.test.ts` — `codexCli` 항목 테스트, runner-aware preflight 로직
- signal/resume 관련 테스트 — `reopen_config`, `config-cancel` pauseReason, `phaseReopenFlags` 경로
- runner 테스트 — Claude runner (interactive + gate), Codex runner (interactive + gate)

### 문서 변경
- `docs/HOW-IT-WORKS.md` — model selection UI, runner 분리, InputManager 설명 추가
