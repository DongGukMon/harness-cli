...
function migrateState(raw: any): HarnessState {
  // phasePresets 누락 → PHASE_DEFAULTS로 초기화
  if (!raw.phasePresets) {
    raw.phasePresets = { ...PHASE_DEFAULTS };
  }
  // lastWorkspacePid 누락 → null
  if (raw.lastWorkspacePid === undefined) {
    raw.lastWorkspacePid = null;
  }
  // 각 preset ID 유효성 검증 — invalid preset → PHASE_DEFAULTS[phase]로 fallback
  for (const [phase, presetId] of Object.entries(raw.phasePresets)) {
    if (!MODEL_PRESETS.find(p => p.id === presetId)) {
...
- `codexPath` 필드는 **제거하지 않고 유지** — 이전 state.json 호환을 위해
- Codex runner는 `codexPath`를 사용하지 않고 standalone `codex` binary를 PATH에서 resolve
- `preflight.ts`에 `codexCli` 항목 추가: `which codex` 또는 `command -v codex`로 검증
- `start.ts`/`resume.ts`에서 `codexPath` 관련 로직:
  - 사용자가 Codex preset을 하나라도 선택한 경우에만 Codex CLI preflight 실행
  - 모든 phase가 Claude preset이면 Codex 검증 스킵
