...
__inner (inner.ts):
  ...
  5. runner-aware preflight — 선택된 preset의 runner union 검증
     실패 시: InputManager.stop() → printError() → state를 paused로 저장 → lock 해제 → exit
...
const onData = (buf: Buffer) => {
  ...
  if (str === '\x03' || str === '\x04') {
    if (this.state === 'configuring') {
      // Pre-loop: phase를 mutate하지 않고 깔끔하게 종료
      this.onConfigCancel?.();  // → paused 저장, lock 해제, exit
    } else {
...
```ts
// 남은 phase: currentPhase부터 7까지 (completed가 아닌 것)
const remainingPhases = REQUIRED_PHASE_KEYS.filter(
