...
- Date: 2026-04-18
- Status: Draft (Phase 1 output, Gate-2 rev-1)
...
- Phase 7 REJECT → **Phase 1 reopen** (풀 플로우는 Phase 3 reopen). 이유: light에서는 spec과 plan이 한 문서이므로, 설계 수정과 계획 수정이 분리되지 않는다. 결합 doc을 재작성해야 impl이 따라간다.
...
| `src/phases/gate.ts` | Phase 7 REJECT 시 `getReopenTarget(flow, gate=7)` 호출 → light면 `1`, full이면 `3` | — |
