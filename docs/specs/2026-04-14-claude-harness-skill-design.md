...
사용자의 글로벌 `~/.claude/CLAUDE.md`에 `harness-lifecycle` 규칙, `~/.claude/settings.json`에 SessionStart 훅 + Codex 퍼미션이 수동 설정되어 있다.

외부 공유가 불가능한 구조다. 새 사용자가 harness를 쓰려면 스킬 파일 복사, 스크립트 배치, CLAUDE.md 편집, settings.json 훅/퍼미션 추가를 수작업으로 해야 한다.
...
| F2 | P1 | `codex-gate-review`가 로컬 캐시 절대 경로 하드코딩 | ADR-6: 새 CLI 명령어 `harness gate-exec`가 런타임에 경로 해석. 스킬에서 절대 경로 완전 제거. 퍼미션 엔트리도 갱신. | [harness gate-exec](#harness-gate-exec), [codex-gate-review 변경](#skillscodex-gate-reviewskillmd), [Permission Migration](#permission-migration) |
| F3 | P1 | CLI↔플러그인 버전 불일치 시 호환성 계약 부재 | ADR-7: `plugin.json`에 `minCliVersion` 선언. CLI 명령어(`gate-exec`, `verify`)가 자체 호출 시 내부에서 자동 검증. 미달 시 차단 메시지. | [Version Compatibility Contract](#version-compatibility-contract) |
...
| ADR-6 | Codex 실행 경로를 CLI 명령어로 래핑 (`harness gate-exec`) | F2 (P1) | 캐시 경로 하드코딩 완전 제거. 퍼미션 엔트리도 `harness gate-exec` 기반으로 갱신. |
| ADR-7 | `minCliVersion` 선언 + CLI 명령어 자체 버전 검증 | F3 (P1) | CLI 명령어(`gate-exec`, `verify`)가 실행 시작 시 자체 버전을 `minCliVersion`과 비교. 스킬이 별도 검증할 필요 없음. |
...
├── src/
│   ├── commands/
│   │   ├── init.ts          ← 새 파일 (ADR-4: marketplace + 훅 + 퍼미션만 설정)
...
**`minCliVersion` 필드** (ADR-7, F3 해소): 이 플러그인이 정상 동작하기 위해 필요한 최소 harness-cli 버전. CLI 명령어가 실행 시작 시 자체적으로 이 값과 현재 CLI 버전을 비교한다.

**참고:** `minCliVersion`은 플러그인 측 선언이다. 실제 검증 로직은 CLI 명령어 내부에 있다 — 자세한 메커니즘은 [Version Compatibility Contract](#version-compatibility-contract) 참조.
...
2. **settings.json 패치** (`~/.claude/settings.json`):
   ...
   - `hooks.SessionStart`에 `harness session-hook` 훅 등록 (기존 레거시 `bash ~/.claude/scripts/harness-session-hook.sh` 엔트리가 있으면 교체)
   - `permissions.allow`에서 레거시 Codex 퍼미션 교체 (아래 [Permission Migration](#permission-migration) 참조)
   - **`enabledPlugins`는 조작하지 않는다** — 이 필드는 Claude Code의 `/plugin install`이 fetch/install 성공 후 자동 관리한다.

3. **결과 출력**: 적용된 변경 사항 + 다음 단계 안내

**`--dry-run`**: 실제 변경 없이 적용될 변경 목록을 JSON diff 형식으로 출력:

```json
{
  "changes": [
    {
      "path": "extraKnownMarketplaces.harness",
      "action": "add",
      "value": {"source": {"source": "github", "repo": "<org>/harness-cli"}}
    },
    {
      "path": "hooks.SessionStart[harness]",
      "action": "add",
      "value": {"type": "command", "command": "harness session-hook"}
    },
    {
      "path": "permissions.allow",
      "action": "replace",
      "old": "Bash(node ~/.claude/plugins/cache/openai-codex/*)",
      "new": "Bash(harness gate-exec *)"
    }
  ],
  "nextSteps": [
...
> **F2 보완:** 스킬에서 절대 경로를 제거하면 `permissions.allow` 엔트리도 갱신해야 한다.

`harness init`은 settings.json의 `permissions.allow` 배열에서 레거시 Codex 퍼미션 엔트리를 검색하고 교체한다:

| Before (레거시) | After (플러그인) |
|-----------------|-----------------|
| `"Bash(node ~/.claude/plugins/cache/openai-codex/*)"` | `"Bash(harness gate-exec *)"` |

교체 로직:
- 배열 내 `"Bash(node ~/.claude/plugins/cache/openai-codex/*)"` 문자열 정확 일치 → `"Bash(harness gate-exec *)"` 교체
- 레거시 엔트리 미존재 + 새 엔트리도 미존재 → 새 엔트리 추가
- 이미 새 엔트리 존재 → 변경 없음
...
SessionStart 훅에서 호출되는 명령어. 기존 `bash ~/.claude/scripts/harness-session-hook.sh`를 대체한다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `type` | `"info"` | Y | 메시지 유형. 유효값: `"info"`. 향후 `"warning"` 등 확장 가능. |
...
**내부 버전 검증** (ADR-7): `gate-exec` 실행 시작 시, CLI 자체 버전과 `plugin.json`의 `minCliVersion`을 비교한다. 불일치 시 명확한 에러 메시지와 함께 exit code 1로 종료한다. 스킬에서 별도 `harness --version` 호출이 불필요하다.
...
package.json의 `version` 필드를 stdout에 출력. 디버깅 및 수동 확인 용도.
...
```
[Plugin: .claude-plugin/plugin.json]       [CLI: package.json]
  minCliVersion: "0.2.0"     ←compare→      version: "0.2.0"
```

### 검증 방식

**CLI 명령어 내부 자동 검증 (ADR-7):**

`harness gate-exec`와 `harness verify`는 실행 시작 시 다음을 수행한다:

1. 플러그인 설치 경로에서 `.claude-plugin/plugin.json`을 찾아 `minCliVersion` 읽기
   - 탐색 순서: `__dirname` 기반 package-local → `~/.claude/plugins/cache/` 내 harness 디렉토리
   - `plugin.json` 미발견 시 → 검증 건너뜀 (로컬 개발/미설치 상태)
2. 현재 CLI 버전 (`package.json#version`)과 semver 비교
3. CLI 버전 < minCliVersion → 차단:
   ```
   Error: harness-cli v0.1.0 is too old for this plugin (requires >= 0.2.0).
   Run: npm update -g harness-cli
   ```
   exit code 1로 종료.

**참고:** 스킬 텍스트에 별도 버전 검증 로직을 삽입하지 않는다. CLI 명령어가 자체적으로 검증하므로 스킬은 `harness gate-exec`나 `harness verify`만 호출하면 된다.
...
> **F1 해소 (P1):** step 2(`harness init`)는 marketplace + 훅 + 퍼미션만 등록. step 3(`/plugin install`)에서 Claude Code가 fetch 성공 후 `enabledPlugins`를 자동 추가. 순서 충돌 없음.

```bash
# Step 1. CLI 설치 (npm 글로벌)
npm install -g harness-cli

# Step 2. 초기 설정 — marketplace 등록 + SessionStart 훅 등록 + 퍼미션 등록 + 의존성 진단
...
### settings.json 변경 상세 (before → after)

Step 2(`harness init`)가 적용하는 `~/.claude/settings.json` 변경 내역:

**hooks.SessionStart — 훅 교체:**
...
"SessionStart": [{
  ...
  "hooks": [{
    "type": "command",
    "command": "bash ~/.claude/scripts/harness-session-hook.sh"
  }]
}]
...
"SessionStart": [{
  ...
  "hooks": [{
    "type": "command",
    "command": "harness session-hook"
  }]
}]
```

**permissions.allow — 퍼미션 교체:**

```jsonc
// Before (레거시):
"allow": [
  "Bash(node ~/.claude/plugins/cache/openai-codex/*)"
]

// After (플러그인):
"allow": [
  "Bash(harness gate-exec *)"
]
```
...
"extraKnownMarketplaces": {
  "harness": {
    "source": {"source": "github", "repo": "<org>/harness-cli"}
  }
}
```
...
# 2. settings.json 갱신 (marketplace 등록 + 훅 교체 + 퍼미션 교체)
harness init
...
| `skills/codex-gate-review/gate-prompts.md` | gate prompt 템플릿 (내용 변경 없음) |
| `scripts/harness-session-hook.sh` | `~/.claude/scripts/`에서 이전 (내용 변경 없음) |
...
| `package.json` | `"files"` 필드에 `.claude-plugin/`, `skills/`, `scripts/harness-session-hook.sh` 추가. `version` bump to `0.2.0` |
...
| R2 | CLI↔플러그인 버전 불일치 | ADR-7: CLI 명령어(`gate-exec`, `verify`)가 실행 시작 시 `minCliVersion`과 자체 버전을 비교. 불일치 시 차단 메시지 + exit 1. 문서화된 업데이트 순서 (CLI 먼저 → 플러그인 나중). |
...
| R5 | `harness init`이 사용자 settings.json의 기존 설정 손상 | JSON 파싱 후 targeted merge. 관련 없는 필드는 변경 불가. `--dry-run`으로 사전 확인 가능 |
