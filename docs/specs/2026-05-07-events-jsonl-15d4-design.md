...
- `status` derivation rule:
  - 기본값 = 마지막 `session_end.ts`의 `status` (없으면 `'unknown'`).
  - 단, 기본값이 `'interrupted'`이고 phase별 **최신** `phase_end.status === 'failed'`인 phase가 하나라도 있으면 `'failed'`로 격상.
  - `session_end.status`가 `'completed'`이면 (사용자가 실패에서 복구한 경우) `'failed'`로 격상하지 않는다.
  - `session_end.status`가 `'paused'`이면 (재개 가능 상태) `'failed'`로 격상하지 않는다.
  - 본 derivation은 `session_end` 스키마(`'completed' | 'paused' | 'interrupted'`)를 변경하지 않는다 — analyzer 한정 분류.
...
9. **Footer** — `_Generated from <eventsPath> · <eventCount> events · ended at <ISO timestamp from last event ts>_`. `malformedLineCount > 0`일 때만 같은 라인 끝에 ` · <N> malformed lines skipped`를 append (그렇지 않으면 해당 토큰 부재).
...
- **fixture-failed**: P5에서 phase_end status='failed' + session_end.status='interrupted' → Phase Summary 마지막 행 finalStatus='failed', 이후 phase는 행 없음, escalation 이벤트 1개 → Escalations 섹션 1줄, RetrospectiveStats.status === 'failed' (derivation rule per Analyzer interface).
