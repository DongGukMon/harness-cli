...
**Timer placement**: in `handleInteractivePhase` (runner.ts), armed after the `logEvent phase_start` call, and **cleared immediately when `runInteractivePhase` settles** (as the very first line after the `await runInteractivePhase(...)` call returns, before any post-processing like `normalizeInteractiveArtifacts` / `getHead` / `writeState`). The branch-specific clears in (a) completed, (b) normal failed, (c) catch, (d) redirect are retained as **backstops** for control-flow paths that don't pass through the post-`await` point. Using `setTimeout` + `ref/unref`: kept referenced so the Node process doesn't exit before it fires during a stuck phase. Rationale: if only branch clears existed, a slow post-completion step (e.g., large artifact normalization) could let the 30s timer fire AFTER Claude has already produced output — false-positive hint.
...
**Implementation note (mock factory updates)**: existing Vitest suites that mock `src/artifact.js` currently expose only `normalizeArtifactCommit` and `runPhase6Preconditions` — adding a new `commitEvalReport` export will break any suite that uses a rigid full-module mock factory. Affected suites to update (widen factory OR switch to partial mock via `vi.importActual`): `tests/phases/runner.test.ts`, `tests/phases/runner-token-capture.test.ts`, `tests/resume-light.test.ts`, `tests/integration/light-flow.test.ts`. Pattern: `vi.mock('../../src/artifact.js', async (importOriginal) => ({ ...(await importOriginal<typeof import('../../src/artifact.js')>()), commitEvalReport: vi.fn(), /* other overrides */ }))` is the safest uniform shape.
