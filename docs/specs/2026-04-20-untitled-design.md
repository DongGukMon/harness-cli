...
5. **R5. Stale sentinel 방어 (재사용 전용 신규 불변식).** `attemptId` 가 재사용되면 기존 `.harness/<runId>/phase-N.done` 파일이 동일 payload(`{{phaseAttemptId}}`)이기 때문에 `checkSentinelFreshness` 만으로는 이전 attempt 의 완료 신호와 이번 reopen attempt 의 완료 신호를 구별할 수 없다. 이를 차단하기 위해 **reopen 으로 interactive phase 를 relaunch 하는 시점에, claude 프로세스를 spawn 하기 직전에 `phase-N.done` 이 존재하면 반드시 삭제한다.** 이 삭제는 resume 경로와 fresh 경로 모두에 적용되어(재진입 공통 경로) 기존 fresh 동작에도 regression 이 없다. 추가로, watchdog 은 삭제 이후 새로 생성된 파일만 관찰하므로 `attemptId` 매칭만으로도 의도한 완료 신호임이 보장된다. 구현은 `handleInteractivePhase` 에서 runner dispatch 직전에 수행한다(Task 4 참조).
...
### D5. Stale sentinel 선삭제 (R5 구현 규정)
- **위치**: `handleInteractivePhase` 에서 `runInteractivePhase` 호출 직전 (runner dispatch 공통 경로).
- **동작**:
  ```ts
  const sentinelPath = path.join(runDir, `phase-${phase}.done`);
  try {
    if (fs.existsSync(sentinelPath)) fs.unlinkSync(sentinelPath);
  } catch { /* best-effort; watchdog will still detect create event */ }
  ```
- **왜 공통 경로에 두는가**: resume 경로 전용이 아니라 모든 interactive relaunch 직전에 수행하면, "이 relaunch 이후 새로 만들어진 sentinel 만 watchdog 가 관찰한다" 는 불변식이 단순해진다. `attemptId` 가 새 UUID 인 fresh 경로에서는 기존에도 payload 가 다르면 stale 로 분류되었으므로 동작 의미 변화 없음. `attemptId` 재사용(resume) 경로에서만 새로운 방어 로직으로 기능.
- **테스트(§D7에 추가)**: (a) reopen resume 경로에서 기존 sentinel 파일(동일 attemptId 내용)이 spawn 직전 삭제되는지, (b) 삭제 실패(파일 없음/권한) 가 swallowed 되는지, (c) watchdog 이 재생성된 sentinel 만 freshness=true 로 판단하는지.
...
(Gate 2 P1 "resume prompt-delivery contract" 및 "stale sentinel" 항목은 각각 §Context & Decisions Resume CLI 계약 / §R5·§D5 에서 본 설계 내에 해결됨.)
...
3. **Task 3 — Runner 분기 + sentinel 선삭제 도입.**
   ...
   - 동일 위치에서 `runInteractivePhase` 호출 직전에 `phase-<N>.done` 존재 시 삭제 (R5/D5).
...
4. **Task 4 — 테스트.**
   - Unit: runner/interactive 분기 테스트 (mock `claudeSessionJsonlExists` + state fixture). Sentinel 선삭제 unit 케이스.
