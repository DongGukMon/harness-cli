...
**Reproduction**: A background tool (`oh-my-codex` / `.omx/`) running alongside Claude wrote `.omx/` into the `.gitignore` file during P6/P7 of rev1. Post-dogfood verification (grep on `~/.claude/projects/.../{P5-rev2-sessionId}.jsonl`) shows **0 occurrences** of `omx` or `.gitignore` in the P5 rev2 Claude session — Claude did NOT touch `.gitignore` in rev2. The dirty state was inherited from outside the Claude-driven scope. When `validatePhaseArtifacts` ran `git status --porcelain`, it saw the non-empty output and returned `false`, marking P5 failed even though the functional diff (tests + cli.py) was committed at `0d9a868`.

**Root-cause attribution (corrected post-advisor review)**: The dirty state is **environmental** (external tool), not harness-internal, and not Claude-error. The P5 validator is functioning as designed; a stricter view is that users are responsible for a clean environment. This finding therefore does **not** belong on the "fix harness" PR — but it remains a legitimate UX observation, because users in realistic dev envs commonly have MCP servers, editors, or CLI companions writing to tracked files. A future enhancement (auto-stage residual edits, or an env-pollution warning in preflight) would improve UX without weakening the validator's core guarantee.
